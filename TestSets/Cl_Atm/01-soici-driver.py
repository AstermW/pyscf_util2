...
for basis in BASIS:
    ...
    os.system("cp %s %s" % (RELDUMP_NAME % (basis), "RELDUMP"))

    for cmin in CMIN:

        kernel(
            True,
            task_name="SOiCI_Cl_small_%s_%.2e" % (basis, cmin),
            fcidump=FCIDUMP_NAME % (basis),
        ...
        kernel(
            True,
            task_name="SOiCI_Cl_large_%s_%.2e" % (basis, cmin),
            fcidump=FCIDUMP_NAME % (basis),
